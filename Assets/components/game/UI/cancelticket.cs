...
public class cancelticket : MonoBehaviour
{
    public void cancel()
    {
        ...
        string command = "UPDATE [taas].[dbo].[tasp] set status='Canceled' WHERE status='Print' and ter_id="+GameObject.FindObjectOfType<userManager>().getUserData().id+"and g_id="+GameObject.FindObjectOfType<claimmanager>().gameid+";";
        string command2 = "SELECT [tot] from [taas].[dbo].[tasp] where status='Canceled' and ter_id=" + GameObject.FindObjectOfType<userManager>().getUserData().id + "and g_id=" + GameObject.FindObjectOfType<claimmanager>().gameid + ";";
